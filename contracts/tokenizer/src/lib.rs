...
use soroban_sdk::{
    ...
    BytesN, Env, IntoVal, MuxedAddress, Symbol, Val, Vec, I256,
};
...
enum DataKey {
    ...
    /// call arrives.
    LastObservedRate,
    /// Blocks `split` only. Every exit stays open.
    Paused,
    PendingAdmin,
...
pub enum Error {
    ...
    Insolvent = 9,
    /// `split` was called while paused. Exits are never blocked.
    Paused = 10,
    NotAuthorized = 11,
    NotPendingAdmin = 12,
    UpgradeNotReady = 13,
...
impl Tokenizer {
    ...
    /// Halts `split` — the only way new PT/YT enters existence. Guardian or
    /// admin, so stopping a live exploit does not wait on a multisig.
    ///
    /// `recombine`, `redeem_at_maturity`, `claim_yield` and `observe_rate` are
    /// never blocked: pausing must not strip a holder of their principal or
    /// their accrued yield.
    ...
    /// Admin only — the deliberate half of the asymmetric pair.
    ...
    /// Moves a non-protocol token out of escrow. SY, PT and YT are all refused:
    /// SY *is* the escrow, and PT/YT arriving here are mid-flight in a flash
    /// route.
    pub fn sweep(env: Env, token_id: Address, to: Address) -> Result<i128, Error> {
        let config = Self::read_config(&env)?;
        config.admin.require_auth();
        Self::require_not_renounced(&env)?;
        if token_id == config.sy_token || token_id == config.pt_token || token_id == config.yt_token
        {
            return Err(Error::ProtectedAsset);
        }
        let balance = token_balance(&env, &token_id, &env.current_contract_address());
        if balance <= 0 {
            return Err(Error::InvalidAmount);
        }
        push_token(&env, &token_id, &to, balance);
        Ok(balance)
    }
    ...
    pub fn split(env: Env, from: Address, sy_amount: i128) -> Result<(i128, i128), Error> {
        from.require_auth();
        let config = Self::read_config(&env)?;
        // The only entry path here. recombine / redeem_at_maturity /
        // claim_yield are exits and are deliberately never gated on pause.
...
mod test {
    ...
    fn admin_transfer_is_two_step() {
        ...
        assert_eq!(fixture.client.pending_admin(), Some(next.clone()));
    ...
    #[test]
    fn pause_blocks_split_only() {
        let fixture = fixture(NOW);
        initialize(&fixture);
        fixture.client.pause();
        assert!(fixture.client.is_paused());

        // split is the only way new PT/YT is created, so it is the only thing
        // pause closes.
        assert!(fixture.client.try_split(&fixture.admin, &100).is_err());

        // Exits stay open. These reach further failures in the unit fixture
        // (placeholder PT/YT addresses), but never Paused (#10) — which is the
        // property under test: pausing must not strip a holder of principal or
        // accrued yield.
        let err = format!(
            "{:?}",
            fixture.client.try_recombine(&fixture.admin, &10, &10)
        );
        assert!(
            !err.contains("#10"),
            "recombine must not be pause-gated: {err}"
        );

        // Rate observation stays permissionless so the maturity freeze cannot
        // be starved by a pause.
        assert!(fixture.client.observe_rate() > 0);
    ...
    fn upgrade_respects_the_timelock() {
        let fixture = fixture(NOW);
        initialize(&fixture);
        let hash = BytesN::from_array(&fixture.env, &[9u8; 32]);
    ...
    #[test]
    fn sweep_refuses_escrow_and_the_pt_yt_pair() {
        let fixture = fixture(NOW);
        initialize(&fixture);
        assert!(fixture
            .client
        ...
        assert!(fixture
            .client
            .try_sweep(&fixture.yt_token, &fixture.admin)
            .is_err());
    }
}
